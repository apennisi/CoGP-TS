...
class ConvertKeypoints:
    ...
    def _convert(self, keypoints, w, h):
        converted_keypoints = list(keypoints[i] for i in config['reorder_map'])
...
class CropPad:
    def __init__(self, pad, center_perterb_max=40, crop_x=config['input_size'], crop_y=config['input_size']):
        self._pad = pad
...
class FlipLeftRight:
    ...
    def _swap_left_right(self, keypoints):
        for r, l in zip(config['swap_right'], config['swap_left']):
...
class FlipUpDown:
    ...
    def _swap_left_right(self, keypoints):
        for r, l in zip(config['swap_up'], config['swap_down']):
            keypoints[r], keypoints[l] = keypoints[l], keypoints[r]
        return keypoints
