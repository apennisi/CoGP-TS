...
from datasets.transformations import ConvertKeypoints, Scale, Rotate, CropPad, FlipLeftRight, FlipUpDown
...
#      net.load_state_dict(torch.load(checkpoint_path, map_location="cuda:0"))
    ...
    sigma = config['sigma']
    path_thickness = config['path_thickness']
    ...
    num_iter = 0
    current_epoch = 0
    drop_after_epoch = config['drop_after_epoch']
    ...
    for epochId in range(current_epoch, epochs):
        ...
        for batch_data in train_loader:
            ...
            loss = 0
            for loss_idx in range(0, len(losses)):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-refinement-stages', type=int, default=config['num_refinement_steps'], help='number of refinement stages')
    ...
    parser.add_argument('--val-after', type=int, default=100,
                        help='number of iterations to run validation')
    args = parser.parse_args()

    checkpoints_folder = config['checkpoint_folder']
